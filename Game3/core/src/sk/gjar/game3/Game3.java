...
public class Game3 extends ApplicationAdapter {
    ...
    public void render() {
        ...
        batch.begin();
        if (isMovingRight) {
            zombieSprite.setPosition(zombieSprite.getX() + Gdx.graphics.getDeltaTime() * 50, zombieSprite.getY());
        } else {
            zombieSprite.setPosition(zombieSprite.getX() - Gdx.graphics.getDeltaTime() * 50, zombieSprite.getY());
        }
        if (zombieSprite.getX() > 6 * TEXTURE_WIDTH) {
            isMovingRight = false;
        }
        if (zombieSprite.getX() < 2 * TEXTURE_WIDTH) {
            isMovingRight = true;
        }
        /* solution 1
        ...
        */
        ...
        for (int i = 0; i < bottomLine1.length; i++) {
            if (bottomLine1[i] > 0) {
                batch.draw(textures[bottomLine1[i] - 1], i * TEXTURE_WIDTH, 0);
            }
        }

        for (int i = 0; i < bottomLine2.length; i++) {
            if (bottomLine2[i] > 0) {
                batch.draw(textures[bottomLine2[i] - 1], i * TEXTURE_WIDTH, 1 * TEXTURE_HEIGHT);
            }
        }

        for (int i = 0; i < bottomLine3.length; i++) {
            if (bottomLine3[i] > 0) {
                batch.draw(textures[bottomLine3[i] - 1], i * TEXTURE_WIDTH, 2 * TEXTURE_HEIGHT);
            }
        }
